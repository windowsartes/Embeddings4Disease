...
class FinalCheckpointCallback(TrainerCallback):
    """
    This callback is used to save trained model in the given file.

    Args:
        checkpoint_dir (str | pathlib.Path): directory where you want to save your model.
    """

    def __init__(self, checkpoint_dir: str | pathlib.Path):
        super().__init__()

        self.checkpoint_dir: pathlib.Path = pathlib.Path(checkpoint_dir)
        utils.create_dir(checkpoint_dir)

    def on_train_end(  # type: ignore
        self,
        args: TrainingArguments,
        state: TrainerState,
        control: TrainerControl,
        **kwargs,
        ...
        """
        This method will be called after the trainin so we can store save trained model.
        ...
        torch.save(checkpoint, self.checkpoint_dir.joinpath("trained_model.pth"))
