...
from embeddings4disease.metrics.head_metrics import MultiLabelHeadMetricComputer
...
class MetricComputerCallback(CustomCallback):
    ...
    def on_evaluate(   # type: ignore
        ...
        **kwargs,
        ...
        """
        if training_state.epoch % self._period == 0:
            ...
            metrics: dict[str, float] = metric_computer.get_metrics_value(model)  # type: ignore[assignment]
            ...
            for metric_name, usage in self._use_metrics.items():
                if usage:
                    ...
                    if self._save_plot:
                        ...
                        plt.savefig(
                            self._metrics_storage_dir.joinpath(
    ...
    def on_evaluate(  # type: ignore
        ...
        **kwargs,
        ...
        Args:
            ...
            model (nn.Module): model you want to evaluate.
        """
        with open(self._loss_storage_dir.joinpath("train_loss_history.json"), "w") as f:
            json.dump(training_state.train_loss_history, f)

        with open(self._loss_storage_dir.joinpath("eval_loss_history.json"), "w") as f:
...
class CheckpointCallback(CustomCallback):
    def __init__(self, checkpoint_storage_dir: str | pathlib.Path):
        ...
        self._checkpoint_storage_dir: pathlib.Path = pathlib.Path(checkpoint_storage_dir)
    ...
    def on_save(  # type: ignore
        ...
        **kwargs,
        ...
        torch.save(checkpoint, self._checkpoint_storage_dir.joinpath("checkpoint.pth"))
    ...
    def on_save(  # type: ignore
        ...
        **kwargs,
        ...
        if training_state.eval_loss_best_value > eval_loss_last_value:
            ...
            torch.save(model.state_dict(), self._checkpoint_storage_dir.joinpath("checkpoint.pt"))
