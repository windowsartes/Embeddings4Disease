...
class MetricComputerCallback(TrainerCallback):
    ...
    def __init__(
        ...
        self._metrics_storage_dir: pathlib.Path = pathlib.Path(metrics_storage_dir)
        utils.create_dir(self._metrics_storage_dir)

        self._period: int = period

        self._device: torch.device = device

        self._use_metrics: dict[str, bool] = use_metrics
        self._use_wandb: bool = use_wandb
        self._save_plot: bool = save_plot

        if self._save_plot:
            sns.set_style("white")
            sns.set_palette("bright")

        for metric, usage in self._use_metrics.items():
            if usage:
        ...
        self._metric_computer: MLMMetricComputer = MLMMetricComputer(
            tokenizer, top_k, dataloader, self._use_metrics
        )
    ...
    def __dump_logs(self, metric_name: str, logs: dict[str, float]) -> None:
        ...
        """
        utils.create_dir(pathlib.Path(self._metrics_storage_dir).joinpath(
            pathlib.Path(metric_name)))

        file_name: pathlib.Path = pathlib.Path(self._metrics_storage_dir).joinpath(
    ...
    def on_evaluate(  # type: ignore
        ...
        **kwargs,
        ...
        """
        if (state.epoch - 1) % self._period == 0:
            metrics: dict[str, float] = self._metric_computer.get_metrics_value(kwargs["model"].to("cpu"))  # type: ignore[assignment]

            kwargs["model"].to(self._device)

            for metric_name, usage in self._use_metrics.items():
                if usage:
                    ...
                    if self._save_plot:
                        ...
                        plt.savefig(self._metrics_storage_dir.joinpath(
                            pathlib.Path(metric_name).joinpath(f"{metric_name}.png")
                        ))

                        plt.close(fig)

            if wandb_installed and self._use_wandb:
                wandb.log(
                    {
                        f"eval/{metric_name}": metrics[metric_name] for metric_name, usage in self._use_metrics.items() if usage
                    }
                )
...
class SaveLossHistoryCallback(TrainerCallback):
    ...
    def __init__(self, loss_storage_dir: str | pathlib.Path, save_plot: bool):
        ...
        self._loss_storage_dir: pathlib.Path = pathlib.Path(loss_storage_dir)
    ...
    def on_evaluate(  # type: ignore
        ...
        **kwargs,
        ...
        with open(self._loss_storage_dir.joinpath("loss_history.json"), "w") as f:
            json.dump(history, f)
        ...
        if self._save_plot:
            ...
            ax.plot(epoch_history, loss_train_history, label="Training", color="red")
            ax.plot(epoch_history, loss_val_history, label="Validation", color="blue")
            ...
            plt.savefig(self._loss_storage_dir.joinpath("loss.png"))
...
class EncoderDecoderMetricPlotsCallback(TrainerCallback):
    def __init__(
        self,
        metrics_storage_dir: str | pathlib.Path,
        use_metrics: dict[str, bool],
        ...
        for metric, usage in self._use_metrics.items():
            if usage:
                pathlib.Path(self._metrics_storage_dir).joinpath(pathlib.Path(metric)).mkdir(parents=True, exist_ok=True)
    ...
    def on_evaluate(  # type: ignore
        ...
        **kwargs,
        ...
        for metric, usage in self._use_metrics.items():
            if usage:
                metric_value: dict[int, float] = {}

                for step in history:
                    if f"eval_{metric}" in step:
                        metric_value[int(step["epoch"])] = step[f"eval_{metric}"]

                file_name: pathlib.Path = pathlib.Path(self._metrics_storage_dir).joinpath(
                    pathlib.Path(metric).joinpath(f"{metric}.json")
                )

                with open(file_name, "w") as file_json:
                    json.dump(metric_value, file_json)
                ...
                if self._save_plot:
                    ...
                    plt.plot(list(metric_value.keys()), list(metric_value.values()), color="blue")
                    ...
                    plt.title(f"Value of {metric} on validation\n during training")
                    plt.xlabel("Number of epochs")

                    file_name_img: pathlib.Path = pathlib.Path(self._metrics_storage_dir).joinpath(
                        pathlib.Path(metric).joinpath(f"{metric}.png")
                    )
